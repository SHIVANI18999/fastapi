...
def feed_page():
    ...
    if response.status_code == 200:
        ...
        for post in posts:
            ...
            like_count = post.get("like_count", 0)  # default to 0 if missing
            like_button = st.button(f"❤️ {like_count}", key=like_key)

            if like_button:
                like_response = requests.post(f"http://localhost:8000/posts/{post['id']}/like", headers=get_headers())
            ...
            comments_response = requests.get(f"http://localhost:8000/posts/{post['id']}/comments", headers=get_headers())
            if comments_response.status_code == 200:
                ...
                for comment in comments:
                    ...
                    with col_c1:
                        st.markdown(f"**{comment['user_id']}**: {comment['description']}")
                    
                    with col_c2:
                        if comment["user_id"] == st.session_state.user["id"]:
                            if st.button("X", key=f"del_comment_{comment['id']}"):
                                delete_response=requests.delete(
                                    f"http://localhost:8000/comments/{comment['id']}",
                                    headers=get_headers()
                                )
            ...
            with st.form(key=f"comment_form_{post['id']}"):
                ...
                if submit_comment and comment_text.strip():
                    post_comment_response = requests.post(
                        f"http://localhost:8000/posts/{post['id']}/createcomment",
                        data={"description": comment_text},
                        headers=get_headers(),
                    )
                    if post_comment_response.status_code in [200, 201]:
